    ...
    async run(client,message,args,prefix){
        ...
        let em = new EmbedBuilder().setColor(client.config.color).setAuthor({name : `${client.user.username} HelpDesk`,iconURL : client.user.displayAvatarURL()}).setDescription(
            `Hey ${message.author} I am ${client.user.username}
             ${client.emoji.arrow} A complete Music Bot for PRO NATION
             ${client.emoji.arrow} Providing you the best quality music
             ${client.emoji.arrow} [<:discord:1083456067230507068> Discord](https://discord.gg/pronation) | [<:youtube:1083456064822976543> Youtube](https://www.youtube.com/channel/UCaa3DT4IWjALWXUMGx5iejw) | [<:instagram:1083456062537084938> Instagram](https://www.instagram.com/pnharsh/)`
        ).addFields({
        ...
        }).setFooter({text : `Developed with ❤️ For PRO NATION` , iconURL : message.guild.iconURL({dynamic : true})}).setThumbnail(message.author.displayAvatarURL({dynamic : true}))
        ...
        ]).setThumbnail(message.author.displayAvatarURL({dynamic : true})).setFooter({text : `Developed with ❤️ For PRO NATION` , iconURL : message.guild.iconURL({dynamic : true})})
